...
figma.ui.onmessage = msg => {
  ...
  if (msg.type === 'create-variables') {
    ...
    function negSizeLookup(napple: number): string {
      // gets string name of root spacer size from negArray[napple]
      if (napple !== 0) {
        let snapple = napple++;
        let strapon = (snapple as unknown) as string;
    ...
    function posSizeLookup(napple: number): string {
    ...
    function getRootValues(baseValue: number, negArray:string[], posArray:string[]){
